...
<div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
  <div class="text-muted"><?= e($student['student_code']) ?></div>
  <div class="d-flex gap-2">
    <a href="/students/<?= (int) $student['id'] ?>/edit" class="btn btn-outline-secondary"><?= e(__('common.edit')) ?></a>
...
<div class="card mb-3">
  <div class="card-body">
    <?php if (!empty($student['photo_path'])): ?>
      <img src="/students/<?= (int) $student['id'] ?>/photo" alt="" style="width:96px; height:96px; object-fit:cover;" class="rounded-circle border mb-3">
    <?php endif; ?>
    <dl class="row mb-0">
...
<div class="card mb-3">
  <div class="card-header d-flex justify-content-between align-items-center">
    <span><?= e(__('students.current_enrollment')) ?></span>
    <div class="d-flex gap-2">
      <?php if ($canReassign): ?>
        <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#reassignModal">
          <?= e(__('students.reassign_class')) ?>
        </button>
      <?php endif; ?>
      <?php if ($canUndo): ?>
        <form method="post" action="/students/<?= (int) $currentEnrollment['id'] ?>/undo-promotion" data-confirm="<?= e(__('students.confirm_undo_promotion')) ?>">
          <?= CsrfMiddleware::field() ?>
          <button type="submit" class="btn btn-sm btn-outline-warning"><?= e(__('students.undo_promotion')) ?></button>
        </form>
      <?php endif; ?>
    </div>
  </div>
  <div class="card-body">
    <?php if ($currentEnrollment === null): ?>
      <p class="text-muted mb-0"><?= e(__('students.no_current_enrollment')) ?></p>
    <?php else: ?>
...
<div class="card">
  <div class="card-header"><?= e(__('students.enrollment_history')) ?></div>
  <?php if (empty($history)): ?>
    <div class="card-body"><p class="text-muted mb-0"><?= e(__('common.no_results')) ?></p></div>
  <?php else: ?>
